...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bike.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bikes.update", [$bike->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('brand') ? 'is-invalid' : '' }}" type="text" name="brand" id="brand" value="{{ old('brand', $bike->brand) }}" required>
                @if($errors->has('brand'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('owner') ? 'is-invalid' : '' }}" name="owner_id" id="owner_id" required>
                    @foreach($owners as $id => $owner)
                        <option value="{{ $id }}" {{ (old('owner_id') ? old('owner_id') : $bike->owner->id ?? '') == $id ? 'selected' : '' }}>{{ $owner }}</option>
                    @endforeach
                </select>
                @if($errors->has('owner'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $bike->quantity) }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Bike::FRONT_LIGHT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('front_light') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="front_light_{{ $key }}" name="front_light" value="{{ $key }}" {{ old('front_light', $bike->front_light) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Bike::REAR_LIGHT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('rear_light') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="rear_light_{{ $key }}" name="rear_light" value="{{ $key }}" {{ old('rear_light', $bike->rear_light) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Bike::SPEED_SENSOR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('speed_sensor') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="speed_sensor_{{ $key }}" name="speed_sensor" value="{{ $key }}" {{ old('speed_sensor', $bike->speed_sensor) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Bike::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $bike->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('longitude') ? 'is-invalid' : '' }}" type="number" name="longitude" id="longitude" value="{{ old('longitude', $bike->longitude) }}" step="0.01" required>
