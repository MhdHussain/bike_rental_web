    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.rentals.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bike') ? 'is-invalid' : '' }}" name="bike_id" id="bike_id" required>
                    @foreach($bikes as $id => $bike)
                        <option value="{{ $id }}" {{ old('bike_id') == $id ? 'selected' : '' }}>{{ $bike }}</option>
                    @endforeach
                </select>
                @if($errors->has('bike'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('period_in_days') ? 'is-invalid' : '' }}" type="text" name="period_in_days" id="period_in_days" value="{{ old('period_in_days', '1') }}">
